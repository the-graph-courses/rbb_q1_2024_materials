...
You can see that cases are recorded from early 2015 to mid 2015. The variables `date_of_hospitalisation` and `date_of_onset` are the most complete (they have no NAs), so we will be using them for visualization in this exercise.
...
First, create a summary data frame showing the number of cases for each **onset date**, split by **outcome** (i.e., calculate the total number of cases grouped by onset date and outcome). Omit NA values from your summary table.
...
The due date for the assignment is **Friday, March 8th at 23:59 PM UTC.**
