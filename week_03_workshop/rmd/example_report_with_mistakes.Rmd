---
title: "Workshop Example report: PROBLEMS"
output:
...
HOWEVER! There are multiple issues with the document that will prevent it from knitting successfully. You mission is to find and fix the issues, and save the report! Follow the instructions in the workshop guide for hints.
...
## Age group distribution per  group

The two categorical variables I chose to look at are age group and . The victims ranged from `r min(col_accidents_clean$age)` to `max(col_accidents_clean$age)` years old.

The plots below show the age group distributions for each .
...
For both es, the age group with the most deaths was the 20 to 24 age group. 